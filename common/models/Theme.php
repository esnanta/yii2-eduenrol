...
class Theme extends BaseTheme
{
    ...
    const LOGO_common_HEADER   = 3;
    const LOGO_common_FOOTER   = 4;
    ...
    public static function getArrayThemeType()
    {
        return [
            ...
            self::LOGO_common_HEADER   => 'Logo common Header',
            self::LOGO_common_FOOTER   => 'Logo common Footer',
    ...
    {
        $directory = Theme . phpstr_replace('frontend', 'common', Yii::getAlias('@webroot')) . self::$path;
        if (!is_dir($directory)) {
        ...
        if (!isset($this->file_name)) {
            switch ($_module) {
                ...
                case ($_module == self::LOGO_FRONTEND_FOOTER):
                    ...
                    break;
                case ($_module == self::LOGO_common_HEADER):
                    $defaultPicture = Yii::$app->urlManager->baseUrl .'/images/unify263-admin/logo.png';
                    break;
                case ($_module == self::LOGO_common_FOOTER):
