...
class Staff extends BaseStaff
{
    ...
    {
        return [
            [['user_id', 'office_id', 'employment_id', 'gender_status', 'active_status',
    ...
    public static function getOneGenderStatus($_module = null)
    {
        if ($_module === null) {
            ...
            return 'Invalid gender status'; // Or throw new \InvalidArgumentException('Gender module cannot be null');
        }

        $arrayModule = self::getArrayGenderStatus();
        $returnValue = 'Unset';

        if (isset($arrayModule[$_module])) {
            switch ($_module) {
    ...
    public static function getOneActiveStatus($_module = null)
    {
        if ($_module === null) {
            // Return a default value or throw an exception
            return 'Invalid module'; // Or throw new \InvalidArgumentException('Module cannot be null');
        }
        ...
        if (isset($arrayModule[$_module])) {
            switch ($_module) {
                case self::ACTIVE_STATUS_YES:
                    $returnValue = LabelHelper::getYes($arrayModule[$_module]);
                    break;
                case self::ACTIVE_STATUS_NO:
                    $returnValue = LabelHelper::getNo($arrayModule[$_module]);
                    break;
                default:
                    $returnValue = LabelHelper::getDefault($returnValue);
